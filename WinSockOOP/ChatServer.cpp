...
void handleMessages (string& name, MsgComm* conn, umap<string, MsgComm*>* connectors) {
...
//	
	while (1) {
		...
		printf ("Some acquiring requests coming ...\n");
		thread  aThread (asyncCommunication, conn, connectors);
		aThread.detach ();
	}
}
